...
from utils import plot_prediction, check_tensor_size, calculate_metrics
from torchviz import make_dot
from torch.utils.tensorboard import SummaryWriter
from model import ComboLoss, FocalTverskyLoss
...
def train(generator, discriminator, train_dataset, test_dataset, args):
    ...
    make_dot(generator(dummy_input), params=dict(generator.named_parameters())).render(os.path.join(args.output_dir, "generator_architecture"), format="png")
    make_dot(discriminator(torch.cat([dummy_input, dummy_input], dim=1)), params=dict(discriminator.named_parameters())).render(os.path.join(args.output_dir, "discriminator_architecture"), format="png")
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (mri_patch, mask_patch) in enumerate(train_loader):
            ...
            loss_g = loss_g_gan + 50 * loss_g_seg + 50 * loss_g_focal
            loss_g.backward()
            optimizer_g.step()
        ...
        avg_val_f1 = val_f1 / num_val_samples
        avg_val_iou = val_iou / num_val_samples

        logger.info(f"Validation - F1: {avg_val_f1:.4f}, IoU: {avg_val_iou:.4f}")
        ...
        writer.add_scalar('Metrics/F1', avg_val_f1, epoch)
